...
trait registrationTrait
{
    ...
    public function registration()
    {
        ...
        if ($emptyNamaPasien == true || $emptyBirthDate == true || $emptyPoli == true || $emptyDokter == true || $emptyDay == true) {
            print $this->messageValidation();
        } else {
            if (!isset($pasien)) {
                if ($noKtpDariPesan == null) {

                    echo $this->sendResponse("Mohon maaf kami tidak dapat menemukan data atas nama : *{$this->getNamaPasien()}* dengan tanggal lahir *$tglLahirFromMessage*. Silahkan ulangi dan masukan no KTP/ no RM pada form di atas.");
                    // \nHubungi Customer care kami di nomor berikut: (021)-7270681")
                } else if (Str::length($noKtpDariPesan) < 5) {
                    echo $this->sendResponse("Mohon maaf kami tidak dapat menemukan data atas nama : *{$this->getNamaPasien()}* dengan tanggal lahir *$tglLahirFromMessage*. Mohon cek kembali no KTP/no RM/tgl. lahir anda.");
                } else {
                    echo $this->sendResponse("Mohon maaf kami tidak dapat menemukan data atas nama : *{$this->getNamaPasien()}* dengan tanggal lahir *$tglLahirFromMessage*.\nUntuk pasien baru silahkan mendaftar di link berikut.\nHubungi Customer care kami di nomor berikut: (021)-7270681");
                }
            } else if (!isset($poli)) {
                echo $this->sendResponse("Mohon cek kembali nama poli yang anda tuju, berikut nama poli yang tersedia:\n-" . implode("\n-", $listPoli));
            } else if (!isset($poliSesuaiJadwal)) {
                echo $this->sendResponse("Mohon maaf hari $hari tidak ada jadwal $poli.\nBerikut jadwal poli $poli:\n{$this->jadwalPoli()}");
            } else if (!isset($dokter)) {
                echo $this->sendResponse("Mohon maaf hari $hari tidak ada dokter a/n. {$this->getDokter()}\nBerikut jadwal " . $poli . " hari " . $hari . ":\n" . $this->jadwalPerPoli(0) . "\n" . $this->jadwalPerPoli(1));
            } else if (!empty($regPeriksa)) {


                return $this->sendResponse("Alhamdulillah! \n*$hari($tglRegistrasi)* anda sudah terdaftar ke *$poli* dengan *{$regPeriksa->dokter['nm_dokter']}*. Praktek pkl. $jamMulai - $jamSelesai dapat antrian no. *{$regPeriksa['no_reg']}* \nSegera informasikan kepada kami jika anda berhalangan hadir. \nTerimakasih.");
            } else {

                //  ambil no rawat terakhir
                $no_rawat_akhir = RegPeriksa::where('tgl_registrasi', $tglLahir)->max('no_rawat');
                ...
                $qrcode = $this->storeQrCode($qrcode, $qrcode);
                // dd($qrcode);
